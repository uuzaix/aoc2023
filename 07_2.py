...
for turn in input:
    ...
    jokers = next((x for x in sortedGroups if "n" in x), "")
    
    if sortedGroups[0] != jokers:
        sortedGroups[0] = sortedGroups[0]+jokers
        sortedWithJokers = [x for x in sortedGroups if x != jokers]
    elif sortedGroups[0] == jokers and len(sortedGroups[0]) != 5: 
        sortedGroups[1] = sortedGroups[1]+jokers
        sortedWithJokers = [x for x in sortedGroups if x != jokers]
    else:
        sortedWithJokers = sortedGroups

    if (len(sortedWithJokers[0]) == 5):
        byType[0].append([lettersHand, bid])
    elif (len(sortedWithJokers[0]) == 4):
        byType[1].append([lettersHand, bid])
    elif (len(sortedWithJokers[0]) == 3 and len(sortedWithJokers[1]) == 2):
        byType[2].append([lettersHand, bid])
    elif (len(sortedWithJokers[0]) == 3):
        byType[3].append([lettersHand, bid])
    elif (len(sortedWithJokers[0]) == 2 and len(sortedWithJokers[1]) == 2):
        byType[4].append([lettersHand, bid])
    elif (len(sortedWithJokers[0]) == 2):
